...
provider "aws" {
  region = "us-west-1"
  # No access_key or secret_key; relies on AWS CLI credentials or IAM role
}
...
resource "aws_iam_role_policy" "ec2_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        Effect = "Allow"
        Action = ["s3:*", "ec2:*"] # Adjust to least privilege
        Resource = "*"
      }
    ]
  })
